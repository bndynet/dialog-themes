var elThemes = document.getElementById("ddlThemes");
for(var theme of themes) {
    if (theme !== "default") {
        addCssFile("../dist/" + theme + ".css", "https://unpkg.com/@bndynet/dialog-themes/dist/" + theme + ".css");
    }
    var option = document.createElement("option");
    option.innerText = theme;
    option.value = (theme === "default" ? "" : theme);
    elThemes.appendChild(option);
}
changeTheme(document.getElementById("ddlThemes"));

function changeTheme(sender) {
    var value = sender.value;
    document.body.className = value;
}
function addCssFile(localPath, externalPath) {
    var el = document.createElement("link");
    el.setAttribute("rel", "stylesheet");
    el.setAttribute("type", "text/css");
    if (location.hostname === '127.0.0.1' || location.hostname === 'localhost') {
        el.setAttribute("href", localPath);
    } else {
        el.setAttribute("href", externalPath);
    }
    document.head.appendChild(el);
}